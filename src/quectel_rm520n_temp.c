 ...
 *
 * The module creates a sysfs directory "quectel_rm520n_temp" with a file
...
static struct kobj_attribute temp_attribute =
    __ATTR(temp, 0644, temp_show, temp_store);
...
{
    // Create the sysfs directory "quectel_rm520n_temp"
    temp_kobj = kobject_create_and_add("quectel_rm520n_temp", kernel_kobj);
    if (!temp_kobj)
        return -ENOMEM;
